...
features_names=[]
for i in range(1100):
   features_names.append("l_"+str(i))
for j in range(1100):
   features_names.append("T_"+str(j))
features_names.append("lambda_max")
features_names.append("totalPMTs")
features_names.append("totalLAPPDs")
explainer = lime.lime_tabular.LimeTabularExplainer(train_x, feature_names=features_names, class_names=['TrueTrackLengthInWater'], verbose=True, mode='regression')
...
exp = explainer.explain_instance(x_transformed[100], model.predict, num_features=5)
exp.save_to_file('explanation100.html')

#Sub-modular pick step
sp_obj = submodular_pick.SubmodularPick(explainer, x_transformed, model.predict, sample_size=40, num_features=5, num_exps_desired=10)

[exp.save_to_file('explanation'+str(i)+'.html') for i, exp in enumerate(sp_obj.sp_explanations)]

print("Predicted value for event 100:", y_predicted[100])
